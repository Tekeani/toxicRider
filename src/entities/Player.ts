...
export class Player extends Entity implements RenderableEntity {
    private game: Game;
    ...
    public isTakingDamage: boolean = false;
    public damageTimer: number = 0;
    public mana: number;
    public maxMana: number;
    public toxicityCooldown: number = 0;
    public toxicityCooldownMax: number = 60;
    private _attackDirection: Direction = 'right';
        ...
        return new Promise<void>((resolve) => {
            ...
            img.onload = () => {
                if (img.complete && img.naturalWidth > 0) {
                    ...
                    (this.spriteSheet as any).framesPerRow = 8;
    ...
    private switchAnimation(newAnimation: Animation): void {
        ...
        if (this.currentAnimation && this.currentAnimation.isPlayingAnimation() && this.currentAnimation !== this.animations.attack) {
            const anim = this.currentAnimation as any;
    ...
    attack(): void {
        if (this.isAttacking || !this.isAlive || this.isBlocking) return;
    ...
    isDeathAnimationComplete(): boolean {
        ...
        if (!this.animations.dead) return true;
