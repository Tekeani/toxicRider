...
import { RenderingSystem, type RenderableEnemy } from '../systems/RenderingSystem';
...
export class RoguelikeScene extends Scene {
    ...
    private waves = [WAVES_CONFIG.WAVE_1, WAVES_CONFIG.WAVE_2, WAVES_CONFIG.WAVE_3];
    private allWavesComplete: boolean = false;
    private tileMap: TileMap | null = null;
    private tilesLoaded: boolean = false;
    private attackFeedbacks: AttackFeedback[] = [];
    private insults: Insult[] = [];
    private insultList: string[] = [
        "T'as vu tes bras ?!",
        "Et dis pas merci !",
        "Go kill yourself !",
    ...
    private usedInsults: string[] = [];
    private waveStartTimer: number = 0;
    private waveStartDelay: number = 5;
    private enemyAttackCooldown: number = 0;
    private enemyAttackIndex: number = 0;
    private _attackPressed: boolean = false;
    private _toxicityPressed: boolean = false;
    private upgradeMenuActive: boolean = false;
    private gameOver: boolean = false;
    private buttonPressed: boolean = false;
    private upgradeButtonPressed = { hp: false, toxicity: false };
    ...
    private setupInput(): void {
        ...
        const cleanup13 = this.game.getInputManager().onKeyDown('1', () => {
            if (this.upgradeMenuActive) this.upgradeStat('hp');
        });
        const cleanup14 = this.game.getInputManager().onKeyDown('2', () => {
            if (this.upgradeMenuActive) this.upgradeStat('toxicity');
        });
        const cleanup15 = this.game.getInputManager().onKeyDown('3', () => {
            if (this.upgradeMenuActive) this.upgradeStat('endurance');
        });
    ...
    async init(): Promise<void> {
        ...
        (this.tileMap as any).generateMap = function() {
        ...
        (this.tileMap as any).generateMap();
    ...
    private playerAttack(): void {
        ...
        if (player.getDamageApplied()) return;
    ...
    private useToxicity(): void {
        if (!this.player || !this.player.useToxicity()) return;
    ...
    private upgradeStat(stat: string): void {
    ...
    render(renderer: Renderer): void {
        ...
        this.enemies.forEach(enemy => {
            if (enemy.isAlive) {
                this.renderingSystem.renderEnemy(enemy as RenderableEnemy);
            }
        });
    ...
    private renderCountdown(renderer: Renderer, size: { width: number; height: number }): void {
        const ctx = renderer.getContext();
    ...
    private renderFeedbacks(renderer: Renderer): void {
        const ctx = renderer.getContext();
    ...
    private renderInsults(renderer: Renderer): void {
        const ctx = renderer.getContext();
    ...
    private renderUpgradeMenu(renderer: Renderer, size: { width: number; height: number }): void {
        const ctx = renderer.getContext();
