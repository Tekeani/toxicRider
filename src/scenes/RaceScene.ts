...
type GameState = 'racing' | 'finished' | 'victory' | 'game_over';
...
export class RaceScene extends Scene {
    private game: Game;
    private gameState: GameState = 'racing';
    private raceTime: number = 0;
    private raceDuration: number = 30;
    private finished: boolean = false;
    private gameOver: boolean = false;
    private bikeX: number = 0;
    private bikeY: number = 0;
    private bikeWidth: number = 80;
    private bikeHeight: number = 80;
    private bikeSpeed: number = 300;
    private BIKE_SPRITE_SIZE: number = 32;
    private bikeCurrentFrame: number = 0;
    private bikeSpriteSheet: HTMLCanvasElement | null = null;
    private roadWidth: number = 400;
    private roadX: number = 0;
    private roadScrollSpeed: number = 500;
    ...
    private fireworkInterval: number = 1.0;
    ...
    private generateBikeSpriteSheet(): void {
        ...
        const drawMotorcycle = (ctx: CanvasRenderingContext2D, x: number, y: number, direction: number): void => {
            ctx.fillStyle = BLACK;
    ...
    private updateVictoryEffects(deltaTime: number): void {
        const size = this.game.getRenderer().getBaseSize();
    ...
    private renderRace(renderer: Renderer): void {
    ...
    private renderBike(renderer: Renderer, size: { width: number; height: number }): void {
        const ctx = renderer.getContext();
        ...
        if (this.gameState === 'game_over') {
            ...
            if (x >= buttonX && x <= buttonX + buttonWidth && y >= buttonY && y <= buttonY + buttonHeight) {
                ...
                setTimeout(() => {
                    this.buttonPressed = false;
                    this.cleanup();
                    const scenes = (this.game as any).scenes;
                    if (scenes) {
